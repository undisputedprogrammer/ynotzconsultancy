<div class="w-full">

    <div class="w-full mt-[60px] lg:mt-[110px]">
        <img class="h-[32px] w-[32px] lg:h-[59px] lg:w-[59px] mx-auto" src="{{asset('images/home/Frame-59.png')}}" alt="">
    </div>
    <div class="w-[85%] mx-auto my-6 lg:my-9 lg:w-[65%]">
        <h1 class=" font-satoshiregular text-4xl lg:text-[5rem] lh-75 text-center">SMALL CHANGES CAN MAKE <span class="text-blue">BIG IMPACT</span>.</h1>
        <p class=" font-satoshiregular text-center mt-6 lg:w-[40%] lg:text-xl mx-auto">We take your challenges, provide accurate solutions and upgrade your business
        </p>
    </div>

    <div class="w-fit mx-auto">
        <a class="h-[43px] px-7 text-white btn-blue text-center font-satoshimedium py-[10px] text-[14px] rounded-lg" href="/services">OUR SERVICES</a>
    </div>




     <x-common.curved-carousel></x-common.curved-carousel>





</div>

